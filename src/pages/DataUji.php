<?php
// Fungsi untuk menambah, mengimpor, menghapus, mengedit, dan mengekspor data uji
addTestData($conn);
importTestData($conn);
deleteTestData($conn);
deleteAllTestData($conn);
editTestData($conn);
exportTestCsv($conn);

// Mengatur paginasi dan statistik
$page = isset($_GET['pagination']) && is_numeric($_GET['pagination']) && $_GET['pagination'] > 0 ? (int)$_GET['pagination'] : 1;  // Menentukan halaman
$records_per_page = 10;  // Menentukan jumlah data per halaman
$dataset = fetchTestDataset($conn, $page, $records_per_page);  // Mengambil data uji berdasarkan halaman
$total_pages = getTestTotalPages($conn, $records_per_page);  // Menghitung total halaman untuk paginasi

// Mengambil total jumlah data uji
$total_query = "SELECT COUNT(*) AS total FROM dataset WHERE type = 'data_uji'";
$total_result = $conn->query($total_query);
$total_count = $total_result ? $total_result->fetch_assoc()['total'] : 0;  // Jumlah total data uji
                                                        ...
                                                        <div class="modal-content">
                                                            ...
                                                            <div class="modal-footer">
                                                                <form method="post" action="">
                                                                    ...
                                                                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Batal</button>
                                                                    <button type="submit" name="delete_test_data" class="btn btn-danger">Hapus</button>
                                                                </form>
                                                            </div>
                                                        </div>
                                                    </div>
